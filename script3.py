...
class SecopApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Label(mode_frame, text="Modo de búsqueda:").pack(side="left", padx=(0, 6))
        ttk.Radiobutton(
            mode_frame, text="Por URL / noticeUID", variable=self.mode_var, value="url"
        ).pack(side="left")
        ttk.Radiobutton(
            mode_frame, text="Por palabra clave", variable=self.mode_var, value="keyword"
        ).pack(side="left", padx=(8, 0))

        # Selector de dataset
        ds_frame = ttk.Frame(container)
        ds_frame.grid(row=3, column=0, sticky="ew", pady=(0, 8))
        ...
        btn_frame.grid(row=4, column=0, sticky="w", pady=(0, 8))
        ...
        status_frame.grid(row=5, column=0, sticky="ew", pady=(0, 6))
        ...
        frame_bottom.grid(row=6, column=0, sticky="nsew")
        ...
        help_text = (
            ...
            "Puedes buscar por URL/noticeUID o por palabra clave (ej. 'policía')."
        )
    ...
    def on_analyze(self):
        ...
        if not query:
            messagebox.showwarning(
                "Falta dato", "Ingresa una URL (modo URL) o una palabra clave (modo keyword)."
            )
            return
    ...
    def _render_records(self, records, dataset_key):
        """Renderiza uno o varios registros."""
        self.text.delete("1.0", tk.END)
        ...
        for ridx, record in enumerate(records, start=1):
            if len(records) > 1:
                self.text.insert(tk.END, f"RESULTADO #{ridx}\n", ("label",))
            for fidx, field in enumerate(fields):
                value = record.get(field, "No disponible")
                if field in currency_fields:
                    value = self._format_currency(value)
                self.text.insert(tk.END, f"{field}: ", ("label",))
                if field in nit_labels and value not in ("No disponible", None):
                    nit_clean = re.sub(r"\D", "", str(value))
                    url = self._build_nit_url(nit_clean) if nit_clean else None
                    self._insert_link(f"{ridx}-{fidx}", value, url)
                    self.text.insert(tk.END, "\n\n")
                else:
                    self.text.insert(tk.END, f"{value}\n\n")
            if len(records) > 1 and ridx < len(records):
                self.text.insert(tk.END, "\n" + "─" * 80 + "\n\n")
        self.text.see("1.0")
        self._hide_loader()
    ...
    def _insert_link(self, idx: str, value: str, url: str | None):
